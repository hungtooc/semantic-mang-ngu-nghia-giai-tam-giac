...
class GRAPH:
    ...
    def addChild(self, node):
        self.children.append(node) # debug: lỗi dòng này :p 
        node.children = []# fix lỗi :D
        node.parent = self

    ''' getAllParents(): return parent : string 
            get all parents P1...Pn '''
    def getAllParents_andMyself(self, list):
        list.append(self.yeuto) # tôi biết tôi là ai
        if self.parent: # biết mẹ bạn là ai không?
            if self.parent.parent: # biết bà tôi là ai không?
                self.parent.getAllParents_andMyself(list)
            else: 
                list.append(self.parent.yeuto)
    ''' getChildrenF1(self): tôi biết con tôi là ai vì tôi sinh ra nó, nhưng tôi chưa chắc biết cháu tôi là ai '''
    ...
    def breadthFirstSearch(self,problem):
        ...
        while not problem.isGoalState(state):
            ...
            for son in successors:
                fringe.push(son) 
            if(not fringe.isEmpty()): state = fringe.pop()
            else: return False
        return state
...
class BAITOAN_TAMGIAC:
    ...
    def BuildGraph_bySemantic(self):
        ...
        while(not self.fringe.isEmpty()):
            ...
            while True:  
                (yeutoCoTheTinh, congthuctinh) = self.getYeuTo_cothetinh(node, actived_yeutos) # trả về yếu tố chưa có trong actived
                if(yeutoCoTheTinh!=-1):
    ...
    def getshallowestDestination(self):
        searchproblem = SearchProblem(self.Startnode, self.yeutocantim ) # Startnode = start_node
    ...
    def GiaiBaiToan(self):
        ...
        desnode = self.getshallowestDestination() # desnode = node đích với bước đi là ngắn nhất
        ...
        if(desnode):
            ...
            for buocgiai in buocgiais:
                loigiai.append("tính " + buocgiai[0] + " thông qua "+ buocgiai[1])
            return loigiai
...
if __name__ == "__main__":
    ...
    baitoan.themCongThuc(congthuc1,congthuc2,congthuc3,congthuc4,congthuc5 , congthuc6, congthuc7,congthuc8, congthuc9, congthuc10,congthuc11, congthuc12, congthuc13, congthuc14, congthuc15)
    start_time = time.time()
    #2 input here
        #bài toán 1
    baitoan.setYeuToDaBiets(canh_b, canh_c, goc_A, goc_B)
    baitoan.setYeuToCanTim(goc_C)
...
''' v0.2
bước 1: xây dựng đồ thị mô tả các bước giải (sử dụng Semantic mod) 
    1. BuildGraph()

bước 2: tìm đường đi ngắn nhất (điểm xuất phát: yeutos đã biết đích: yeutocantim)
'''